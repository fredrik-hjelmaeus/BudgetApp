...
describe('MonthSavingsSummary unit tests', () => {
  ...
  test.only('editing name and number on piggybank saving works correctly', async () => {
    ...
    const editedSavingPresetNumber = await screen.findByRole('button', { name: /999/i });
    const editedSavingPresetName = await screen.findByRole('button', { name: /trip/i });
  ...
  test.skip('editing name,number and category on saving works correctly', async () => {
    ...
    const editedSavingPreset = await screen.findByRole('button', { name: /999/i });
