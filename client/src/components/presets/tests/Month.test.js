...
describe("Summation functionality", () => {
  ...
  test("Add preset to overhead expense works & updates all summation-fields", async () => {
    ...
    //Month Expenses: -1255
    const MonthExpenses =
      screen.getByText(/month expenses:/i).children[0].textContent;
    expect(MonthExpenses).toBe("-1255");

    //Month Balance: -456
    const MonthBalance =
      screen.getByText(/month balance/i).parentElement.children[1].textContent;
    expect(MonthBalance).toBe("Balance Month:-456");

    //Month Savings: 0
    const MonthSavings =
      screen.getByText(/month savings:/i).children[0].textContent;
  ...
  test("Adding overhead presetvalues through upload csv dialog updates all summation-fields correctly", async () => {
    ...
    const filepath = path.resolve(
      __dirname,
      "../../_data/csv_testfiles/RFC4180.csv"
    );
    ...
    expect(
      screen.getByRole("heading", { name: /select csv fields/i })
    ).toBeInTheDocument();
    ...
    //Month Income:
    const monthIncomeSum = (await screen.findByText("Month Income:"))
  ...
  test("Editing overhead expense presetvalues updates all summation-fields", async () => {
    ...
    const AccountBalance =
      screen.getByText("Month Balance:").children[0].textContent;
  ...
  test("Editing overhead INCOME TO EXPENSE presetvalues updates all summation-fields and moves preset", async () => {
    ...
    expect(
      preset.parentElement.parentElement.parentElement.parentElement
  ...
  test("Buy purchase updates all summation-fields and converts preset-type to overhead expense", async () => {
    ...
    const presets_beforeClickingBUY = await screen.findAllByTestId(
      "presetitem"
    );
    ...
    expect(
      presets_afterClickingBUY.length - presets_beforeClickingBUY.length
    ).toBe(1);
  ...
  test("Add piggybank saving to a purchase updates all summation-fields", async () => {
    ...
    const preset = presets.find(
      (b) => b.getAttribute("class") === "text-orange btn-form"
    );
    expect(preset).toBeInTheDocument();
    ...
    const monthSavings =
      screen.getByText("Month Savings:").children[0].textContent;
  ...
  test("Add & Edit saving presetvalues updates all summation-fields", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: /20000/i })
    ).toBeInTheDocument();
    expect(
      await (
        await screen.findAllByAltText("Travel icon")
      ).find((i) => i.name === "edit category")
    ).toBeInTheDocument();
    ...
    const monthBalanceValueEdit =
      screen.getByText("Month Balance:").textContent;
    expect(monthBalanceValueEdit).toBe("Month Balance:-456"); // 544 - 1000
    const BalanceByCategory_TravelFieldEdit =
      screen.getByText("Travel:").children[0].textContent;
    expect(BalanceByCategory_TravelFieldEdit).toBe("-255");
    const monthSavingsEdit =
      screen.getByText("Month Savings:").children[0].textContent;
  ...
  test("Delete saving presetvalues updates all summation-fields", async () => {
    ...
    const presetToDeleteBtn =
      presetToDelete.parentElement.parentElement.parentElement.children[3]
        .children[0];
  ...
  >
    ...
    // find the deletebutton in the html-tree and click it
    const presetToDeleteBtn =
      preset.parentElement.parentElement.children[4].children[0];
    fireEvent.click(presetToDeleteBtn);
  ...
  test("Change inside addtopiggybankmodal updates correctly sums everywhere when submitted", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: /544/i })
    ).toBeInTheDocument();
...
describe("PresetForm interaction", () => {
  ...
  test("Creating a saving-preset only works if there is a surplus in month", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: /savingone/i })
    ).toBeInTheDocument();
    ...
    expect(
      (await screen.findByText(/savingone/i)).parentElement.parentElement
        .parentElement.parentElement.childElementCount
    ).toBe(2);
    expect(
      screen.getByText("Insufficient month surplus for this saving")
    ).toBeInTheDocument();
  });
});
...
describe("Purchases interaction", () => {
  ...
  test("Delete purchase and its piggybank savings", async () => {
    ...
    // delete purchase
    const purchaseDeleteButton = await screen.findByTestId(
      "purchase_item_delete_button"
    );
  ...
  test("Delete piggybank saving updates purchase-month-estimation", async () => {
    ...
    ).parentElement.parentElement.children[1].children[1].children[0]
      .textContent;
  ...
  test("Purchases displays/updates correct when edited", async () => {
    const purchaseNameField =
      screen.getByTestId("purchaseitem").children[0].children[0];
    ...
    expect(monthIncomeSum).toBe("799");
    // Account Balance
    const AccountBalance = (await screen.findByText("Month Income:"))
      .parentElement.children[3].children[0].textContent;
  ...
  test("Purchase displays/updates correct when summation of presets change", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: /income_preset/i })
    ).toBeInTheDocument();
  ...
  test("Click on piggybank or monthsleft button activates AddtoPiggybankModal ", async () => {
    ...
    // expect addtopiggybankmodal to have been opened
    expect(
      await screen.findByRole("heading", { name: /amount to save/i })
    ).toBeInTheDocument();
    // close the modal
